            ...
            try{
                ...
                if(username != null && password != null){
                    ...
                    if(userRegistered != null){
                        String formName = request.getParameter("formName"), formId = request.getParameter("formId");
                        CodificationMap decoder = new CodificationMap();
                        formName = decoder.DecodificateString(formName);
                        int questionNumber = Integer.parseInt(request.getParameter("questionNumber"));
                        if(formName != null && questionNumber > 0 && formId != null){
                            Boolean created = _context.EditForm(username, password, formName, questionNumber,formId);
                            if (!created) out.println(format.GetLastError());
                            else out.println(format.GetResponseInJson("ok", "1", "Completed"));
                        }else{
