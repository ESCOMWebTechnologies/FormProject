 ...
 * @author miku
 */
@WebServlet(name = "GetFormResponse", urlPatterns = {"/Forms/api/GetFormResponse"})
public class GetFormResponse extends HttpServlet {
        ...
        if(connected){
            try{
                ...
                if(username != null && password != null){
                    ...
                    ResultSet set = _context.FirstOrDefault("", "users", String.format("username='%s' and password='%s';",username,password));
                    User userRegistered = _context.GetUser(set);
                    if(userRegistered != null){
                        String formId = request.getParameter("formId");
                        if(formId != null){
                            Pair<Form,Vector<Question>> data = _context.GetFormQuestions(formId);
                            if (data == null) out.println(format.GetResponseInJson("fail","2",_context.GetLastError()));
                            else out.println(format.GetFormAndQuestions("ok", "1", data));
                        }else{
